        ...
        switch ($format) {
            case 'btn-group':
                ...
                foreach ($curent_btn_area as $item) {
                    ...
                    if (!staff_has_permission($controller_name, $item)) continue;
...
if (!function_exists('show_bulk_btn_action')) {
    function show_bulk_btn_action($controller_name, $params = '')
    {
        ...
        foreach ($curent_btn_area as $item) {
            ...
            if (!staff_has_permission($controller_name, $permission_type) && in_array($permission_type, ['change_status', 'export', 'unread', 'delete', 'closed', 'cancel', 'copy_api_order_id', 'copy_api_refill_id', 'resend', 'empty'])) continue;
    ...
    function show_view_ticket_button_group($controller_name, $item = [])
    {
        ...
        if ($dropdowns) {
            ...
            foreach ($dropdowns as $key => $dropdown) {
                if (!staff_has_permission($controller_name, 'unread') && $key == 'unread') continue;
